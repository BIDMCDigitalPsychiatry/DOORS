...
export const createEvent = (data: any) => async dispatch => {
  const response = await axios.post<{ event: Event }>('/api/calendar/events/new', data);

  dispatch(slice.actions.createEvent(response.data));
};

export const selectEvent = (eventId?: string) => async dispatch => {
  dispatch(slice.actions.selectEvent({ eventId }));
};

export const updateEvent = (eventId: string, update: any) => async dispatch => {
...
export const deleteEvent = (eventId: string) => async dispatch => {
...
export const selectRange = (start: Date, end: Date) => dispatch => {
  dispatch(
