...
const getInitialValues = (event?: any, range?: { start: number; end: number }) => {
  if (event) {
    return _.merge(
      {},
      {
...
const AddEditEventForm: FC<AddEditEventModalProps> = ({ event, onAddComplete, onCancel, onDeleteComplete, onEditComplete, range }) => {
  ...
  return (
    <Formik
      ...
      validationSchema={Yup.object().shape({
        ...
        end: Yup.date().when('start', (start: Date, schema: any) => start && schema.min(start, 'End date must be later than start date')),
        start: Yup.date(),
    ...
    >
      {({ errors, handleBlur, handleChange, handleSubmit, isSubmitting, setFieldTouched, setFieldValue, touched, values }) => (
        <form onSubmit={handleSubmit}>
          ...
          <Box p={2} display='flex' alignItems='center'>
            ...
            <Button variant='contained' type='submit' disabled={isSubmitting} color='secondary' className={classes.confirmButton}>
              Confirm
            </Button>
          </Box>
        </form>
